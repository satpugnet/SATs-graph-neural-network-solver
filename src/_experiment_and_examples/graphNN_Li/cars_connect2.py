...
for i in range(1, 29):
    ...
    sdf[:n] = sdf_np
    print(X.shape, boundary_np.shape)
    x = torch.cat([X, boundary, sdf], dim=1)
    # mask_index = torch.tensor(range(n), dtype=torch.long)
    valid = (boundary_np==0)
    valid_index = np.where(valid)[0]
    mask_index = torch.tensor(valid_index, dtype=torch.long)
    sflow_true = sflow_true[valid_index]
    data = Data(x=x, y=sflow_true, edge_index=edge_index, edge_attr=edge_attr, mask_index=mask_index)
    dataset.append(data)
...
path = "car_data/out_local_connect_net"
...
np.savetxt(path + "/train_loss_mp.txt", train_loss)
np.savetxt(path + "/test_loss_mp.txt", test_loss)
