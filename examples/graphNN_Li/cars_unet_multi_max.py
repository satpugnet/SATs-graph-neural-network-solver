...
class Net_multi(torch.nn.Module):
    ...
    def forward(self, data):
        ...
        data1 = Data(x=x, edge_index=edge_index, batch=None, pos=None)
        cluster = get_cluster(n_x, n_y).to(device)
        data1 = max_pool(cluster, data1)
        n_x1 = n_x//2
        n_y1 = n_y//2
        ...
        data2 = Data(x=x1, edge_index=edge_index1, batch=None, pos=None)
        cluster = get_cluster(n_x1, n_y1).to(device)
        data2 = max_pool(cluster, data1)
        n_x2 = n_x1//2
        n_y2 = n_y1//2
        ...
        ### step4, depth 1
        x1_up = self.conv4(x1_up, edge_index1)
        x1_up = F.relu(x1_up)
